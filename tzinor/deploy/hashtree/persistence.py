...
class HashtreePersistence:
    ...
    def save_checkpoint(self, checkpoint_path: str, metadata: dict) -> Optional[str]:
        ...
        try:
            ...
            # 3. Trigger background upload to Blossom servers
    ...
    def _background_blossom_push(self, root_hash: str):
        """Triggers the push to Blossom servers for decentralized CDN distribution."""
        for server in self.blossom_servers:
            self.logger.info(f"Pushing content hash {root_hash} to Blossom server: {server}")
            try:
                # Use htree push to send the blob to the Blossom server
                subprocess.run(
                    [self.htree_bin, "push", "--server", server, root_hash],
                    check=True, capture_output=True
                )
            except Exception as e:
                self.logger.warning(f"Failed to push to Blossom server {server}: {e}")
