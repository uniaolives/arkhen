...
class ArkhenAPIMonitor:
    ...
    def generate_logit_bias(self, vocabulary_sample: List[int]) -> Dict[int, float]:
        ...
        """
        # Em APIs fechadas, não temos os gradientes. Simulamos o viés
        # penalizando tokens de baixa entropia se o damping for baixo.
        bias = {}
        for token_id in vocabulary_sample:
            # Viés heurístico: favorece diversidade quando a gravidade é alta
            bias[token_id] = 2.0  # Boost de +2.0 para tokens selecionados
        return bias
